...
class Command(BaseCommand):
    ...
    def _seed_products(self, categories: dict[str, int]) -> dict[str, int]:
        rows = [
            ("Milk 1L", "Pasteurized whole milk, 3.2% fat", "Dairy"),
            ("Yogurt 200g", "Natural plain yogurt", "Dairy"),
            ("Cheese 200g", "Mild gouda, sliced", "Dairy"),
            ("White Bread", "Classic loaf, 500g", "Bakery"),
            ("Croissant", "Butter croissant, single", "Bakery"),
            ("Whole Wheat Bread", "Sliced loaf, 500g", "Bakery"),
            ("Cola 1.5L", "Carbonated soft drink", "Beverages"),
            ("Orange Juice 1L", "Not-from-concentrate", "Beverages"),
            ("Mineral Water 1.5L", "Still spring water", "Beverages"),
            ("Beer 0.5L", "Lager, 4.7% ABV", "Beverages"),
            ("Chips 100g", "Salted potato chips", "Snacks"),
            ("Chocolate Bar", "Milk chocolate, 90g", "Snacks"),
            ("Crackers 200g", "Salted crackers", "Snacks"),
            ("Soap", "Bar soap, lavender", "Household"),
            ("Toilet Paper 8pk", "2-ply, 8 rolls", "Household"),
        ]
        ids = {}
        for name, characteristics, category in rows:
            ids[name] = execute_insert_returning(
                """
                INSERT INTO products_product (product_name, characteristics, category_id)
                VALUES (%s, %s, %s) RETURNING id
                """,
                [name, characteristics, categories[category]],
            )
        return ids
    ...
    def _seed_employees(self) -> dict[str, int]:
        rows = [
            ("Manager",  "Shevchenko", "Olena",   "Petrivna",  Decimal("45000.00"), date(1985, 4, 12),  date(2018, 3, 1),  "+380501112233", "Kyiv",   "Khreshchatyk 1",   "01001"),
            ("Manager",  "Kovalenko",  "Andriy",  "Vasylovych",Decimal("42000.00"), date(1980, 7, 23),  date(2019, 6, 15), "+380502223344", "Kyiv",   "Sahaidachnoho 5",  "04070"),
            ("Cashier",  "Bondar",     "Iryna",   "Mykolaivna",Decimal("18000.00"), date(1995, 2, 5),   date(2022, 9, 1),  "+380503334455", "Kyiv",   "Velyka Vasylkivska 30", "01004"),
            ("Cashier",  "Tkachenko",  "Yulia",   None,        Decimal("17500.00"), date(1998, 11, 18), date(2023, 1, 10), "+380504445566", "Kyiv",   "Lva Tolstoho 9",   "01032"),
            ("Cashier",  "Melnyk",     "Dmytro",  "Ivanovych", Decimal("18500.00"), date(1992, 9, 30),  date(2021, 4, 20), "+380505556677", "Kyiv",   "Antonovycha 50",   "01033"),
            ("Cashier",  "Sydorenko",  "Kateryna","Olehivna",  Decimal("17800.00"), date(2000, 6, 14),  date(2024, 2, 1),  "+380506667788", "Kyiv",   "Velyka Zhytomyrska 22", "01025"),
        ]
        ids: dict[str, int] = {}
        for role, surname, name, patronymic, salary, dob, dos, phone, city, street, zip_code in rows:
            emp_id = execute_insert_returning(
                ...
                RETURNING id
                """,
                [role, surname, name, patronymic, salary, dob, dos, phone, city, street, zip_code],
            )
            ids[surname] = emp_id
        return ids
    ...
    def _seed_customers(self) -> list[int]:
        rows = [
            ("Petrenko",   "Olha",      "Stepanivna",  "+380671234567", "Kyiv",    "Shevchenka 12",   "01001",  5),
            ("Ivanov",     "Maksym",    "Yurievich",   "+380672345678", "Kyiv",    "Franka 4",        "01030", 10),
            ("Yakovenko",  "Nina",      None,          "+380673456789", None,       None,              None,    15),
            ("Hryhoriev",  "Roman",     "Petrovich",   "+380674567890", "Lviv",    "Rynok 7",         "79008",  5),
            ("Symonenko",  "Anna",      "Andriivna",   "+380675678901", "Kyiv",    "Pushkinska 18",   "01004", 20),
            ("Lysenko",    "Vasyl",     "Mykolayovych","+380676789012", "Odesa",   "Derybasivska 3",  "65000", 10),
            ("Hrytsenko",  "Tamara",    "Ivanivna",    "+380677890123", "Kyiv",    "Saksahanskoho 9", "01032",  0),
            ("Marchenko",  "Yevhen",    "Borysovych",  "+380678901234", "Kyiv",    "Reitarska 35",    "01054", 15),
        ]
        ids: list[int] = []
    ...
    def _seed_checks_and_sales(
        ...
        for offset, surname, cust_idx, line_items in plan:
            ...
            check_id = execute_insert_returning(
                ...
                RETURNING id
                """,
                [employees[surname], card_id, print_date, sum_total.quantize(Decimal("0.0001")), vat],
            )
